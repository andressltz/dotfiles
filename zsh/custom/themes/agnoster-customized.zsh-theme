...
prompt_dir_name() {
  local current_dir icon="📂"
  if [[ $AGNOSTER_GIT_INLINE == 'true' ]] && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    current_dir=$(basename "$(git_toplevel)")
  else
    current_dir="${PWD##*/}"
  fi
  prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$icon $current_dir"
}
...
prompt_java() {
  if command -v java &>/dev/null; then
    if [ -f pom.xml ] || git ls-files --error-unmatch pom.xml >/dev/null 2>&1; then
      ...
      if [[ -n "$short_version" ]]; then
        prompt_segment "$AGNOSTER_STATUS_JOB_FG" "$AGNOSTER_DIR_FG" "☕️ $short_version"
      fi
    fi
  fi
}
...
prompt_node() {
  ...
  if command -v node > /dev/null 2>&1; then
    if [ -f package.json ] || git ls-files --error-unmatch package.json >/dev/null 2>&1; then
      local version
      version=$(node -v | cut -d. -f1,2)  # Ex: v18.19
      if [[ -n "$version" ]]; then
